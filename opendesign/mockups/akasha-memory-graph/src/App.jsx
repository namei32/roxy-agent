...
const STORAGE_KEY = 'roxy.akasha.graph-prototype.v2';
const DEFAULT_OVERVIEW = { expandedGroup: null, groupPage: 0, searchPage: 0 };
const DEFAULT_LOCAL = { filter: 'all', page: 0, focus: true };
const taskTips = { find: '搜索“北极星计划”，找到尚未展开的记忆并核对原文。', shared: '这段记忆同时属于两个组。点选组节点，沿关联核对来源。', crowded: '这个组有很多成员。翻到下一批，选中一段记忆并查看详情。' };
const steps = [
  { id: 'overview', number: '01', name: '图概览', subtitle: '从全貌找到一段记忆', tip: '点选关联组展开成员，或搜索尚未出现在图中的记忆。' },
...
function savedState() {
  try {
    ...
    return { view: steps.some(s => s.id === saved.view) ? saved.view : 'overview', scenario: ['standard','dense','large','empty','error'].includes(saved.scenario) ? saved.scenario : 'dense', rootId: typeof saved.rootId === 'string' ? saved.rootId : 't1', selectedId: typeof saved.selectedId === 'string' ? saved.selectedId : 't1', detailId: typeof saved.detailId === 'string' ? saved.detailId : 't1', revision: Number.isInteger(saved.revision) ? Math.min(99, Math.max(12,saved.revision)) : 12, search: typeof saved.search === 'string' ? saved.search : '', overviewState: { ...DEFAULT_OVERVIEW, ...saved.overviewState }, localState: { ...DEFAULT_LOCAL, ...saved.localState } };
  } catch { return { view: 'overview', scenario: 'dense', rootId: 'dense-1', selectedId: 't1', detailId: 't1', revision: 12 }; }
}
...
function App() {
  ...
  const [overviewState, setOverviewState] = useState(initial.overviewState || DEFAULT_OVERVIEW);
  const [localState, setLocalState] = useState(initial.localState || DEFAULT_LOCAL);
  ...
  useEffect(() => {
    try { localStorage.setItem(STORAGE_KEY, JSON.stringify({ view, scenario, rootId: currentRoot || 't1', selectedId: currentSelected || 't1', detailId: detailId || 't1', revision, search, overviewState, localState })); } catch { /* Private mode still supports this in-memory demo. */ }
  }, [view, scenario, currentRoot, currentSelected, detailId, revision, search, overviewState, localState]);
  ...
  useEffect(() => {
    if (activeView !== 'overview' || !overviewState.expandedGroup) return;
    const content = contentRef.current, heading = content?.querySelector('.overview-mode-line');
    if (heading) content.scrollTo({ top: content.scrollTop + heading.getBoundingClientRect().top - content.getBoundingClientRect().top - 8 });
  }, [overviewState.expandedGroup, activeView, status]);
  ...
  const changeScenario = value => {
    ...
    setOverviewState(DEFAULT_OVERVIEW); setLocalState(DEFAULT_LOCAL); setTask('');
  ...
  const openLocal = id => {
    ...
    setLocalState(DEFAULT_LOCAL); setRootId(id); setSelectedId(id); setTrail([id]); setView('local');
  };
  const centerOn = id => {
    if (stale) return notify('快照已变化，请先刷新后继续探索');
    setLocalState(DEFAULT_LOCAL); setRootId(id); setSelectedId(id); setTrail(items => [...items, id].slice(-8));
  };
  ...
  const reset = () => { changeScenario('dense'); setReload(value=>value+1); notify('已回到第一屏'); };

  const changeOverview = value => { if (stale) return notify('请先刷新，再展开或翻页'); setOverviewState(value); };
  const changeLocal = value => { if (stale) return notify('请先刷新，再更改探索范围'); setLocalState(value); };
  const startTask = kind => {
    changeScenario('dense'); setTask(kind);
    if (kind !== 'find') {
      const id = kind === 'shared' ? 'dense-12' : 'dh0';
      setRootId(id); setSelectedId(id); setTrail([id]); setDetailStack([id]); setView('local');
    }
  };

  return <div className="prototype-shell">
    <header className="studio-header"><a className="studio-brand" href="../../../opendesign/" aria-label="打开原型目录"><span className="brand-glyph"><Network size={20}/></span><strong>roxy</strong><span className="brand-separator"/><span>AKASHA</span></a><span className="studio-version"><span/>交互原型 · 02</span></header>
    <main className="prototype-stage">
      ...
      <section className="device-stage" aria-label="手机原型">
        ...
        <div className="phone-frame"><div className="phone-camera" aria-hidden="true"/><div className="phone-display"><div className="phone-status" aria-hidden="true"><span>9:41</span><div><Signal size={14}/><Wifi size={14}/><BatteryFull size={19}/></div></div>
          ...
          <div className="phone-content" ref={contentRef} key={activeView}>
            {activeView === 'overview' && <Overview fixture={fixture} search={search} onSearch={value => { setSearch(value); setOverviewState(s => ({ ...s, searchPage: 0 })); }} onOpen={openLocal} onHelp={showHelp} onRefresh={refresh} status={status} state={overviewState} onState={changeOverview}/>}
            {activeView === 'local' && <LocalGraph key={currentRoot} fixture={fixture} rootId={currentRoot} selectedId={currentSelected} onSelect={setSelectedId} onCenter={centerOn} onHelp={showHelp} trail={trail.filter(hasNode)} state={localState} onState={changeLocal}/>}
            {activeView === 'detail' && <Detail key={detailId} fixture={fixture} nodeId={detailId} stale={stale} onLocate={openLocal} onDetails={id => { if (stale) return notify('请先刷新示例快照'); setDetailStack(stack=>[...stack,id]); }} notify={notify}/>}
          </div>
      ...
      <aside className="demo-panel"><div className="demo-panel-heading"><span className="eyebrow">TRY IT YOURSELF</span><span className="panel-rule"/></div><h2>{step.name}</h2><p className="current-tip">{task ? taskTips[task] : step.tip}</p><div className="demo-control"><label htmlFor="scenario">演示场景</label><select id="scenario" value={scenario} onChange={e=>changeScenario(e.target.value)}><option value="standard">普通记忆图</option><option value="dense">密集图 · 160 段记忆</option><option value="large">大图 · 1,000 段记忆</option><option value="empty">空图</option><option value="error">读取失败</option></select></div>
        ...
        <div className="task-walkthrough"><span className="mini-label">试走三个任务</span>{[['find','寻找未展开记忆'],['shared','查看共享成员'],['crowded','探索高连接组']].map(([id,label],i) => <button type="button" key={id} aria-pressed={task === id} onClick={() => startTask(id)}><span>{String(i+1).padStart(2,'0')}</span>{label}<ChevronRight size={15}/></button>)}</div>
        <a className="classic-link" href="classic.html">查看第一轮原型对照<ArrowRight size={15}/></a>
