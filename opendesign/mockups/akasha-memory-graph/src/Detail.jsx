...
import { IconButton, MemoryRow, Pager, SnapshotLine } from './components.jsx';
import { neighbors, relationLabel, typeLabel } from './data.js';
import { memberships, paginate } from './projection.js';

export default function Detail({ fixture, nodeId, stale, onLocate, onDetails, notify }) {
  ...
  const batch = paginate(related, relatedPage, 10);
  const groups = node.type === 'turn' ? memberships(fixture,node.id) : [];
  const copy = async () => {
    try {
  ...
  return <div className="screen detail-screen" data-screen="detail">
    ...
    {groups.length > 1 && <section className="shared-membership"><p>同一段记忆，同时属于 {groups.length} 个关联组</p><div>{groups.map(group => <button type="button" key={group.id} onClick={() => onDetails(group.id)}>{group.title}<ArrowUpRight size={14}/></button>)}</div></section>}
    ...
    <section className="detail-relations"><div className="section-heading"><h2>{node.type === 'hub' ? '组内记忆' : '这段记忆的关联'}</h2><span>{related.length} 条</span></div><Pager batch={batch} subject="详情关联" onPage={page => stale ? notify('请先刷新后翻页') : setRelatedPage(page)}/>{batch.items.map(({node: n, edge}) => <MemoryRow key={edge.id} node={n} compact subtitle={relationLabel(edge.type)} right={edge.weight.toFixed(2)} onClick={() => onDetails(n.id)}/>)}<p className="strength-note">数值为关联强度，不代表事实可信度。</p></section>
