...
def load_config(
    ...
    *,
    ...
    _reject_plugin_owned_configuration(data)
...
def _reject_plugin_owned_configuration(data: dict) -> None:
    """Fail loudly when a plugin-owned setting remains in Core config."""

    if "notes_bridge" in data:
        raise ValueError(
            "notes_bridge 已迁移到 Apple Notes 插件的 config.local.toml；"
            "请从 Core config.toml 删除 [notes_bridge]"
        )
