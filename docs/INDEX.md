...
| Akasha 手机端记忆图 | [首版功能约定（proposed）](design/akasha-mobile-memory-graph.md) → [开发准备与镜像基线](design/akasha-mobile-memory-graph-readiness.md) → 上方 Akasha 路由及下方移动端路由 | `plugins/akasha/plugin.py`、`plugins/akasha/inspector.py`、`plugins/akasha/mobile_ui.js`、`scripts/check_akasha_v2_mirror.py`；先核对已记录的镜像漂移 |
