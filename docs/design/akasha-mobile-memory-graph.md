...
- 状态：proposed；本轮完成 Read、Ownership、Isolate、Contract，不包含原型或功能实现。
- 日期：2026-09-05
...
以下是拟实施功能的归属；本轮实际交付属于 `change_type: docs`、`semantic_delta: none`。
...
下一步只需用三屏原型和小规模数据探针收敛：概览选择规则、Hub 稳定身份、发布版本校验、长正文详情分段和上述预算。
具体 API 名称、字段与布局在该阶段固定。原型使用合成数据，不用真实用户会话作为公开样例。
...
本轮回滚只涉及新增设计文档和索引链接；后续代码回滚恢复插件版本，不恢复或覆盖用户数据库。
