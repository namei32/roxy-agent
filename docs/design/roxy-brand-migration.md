...
本节替代前文对旧 Android 下载 URL 的保留要求。维护者要求只保留必要引用。当前页面、文档和展示示例移除原项目 QQ 交流群、群二维码、联系邮箱及原应用下载/讨论入口；不提供未经维护者确认的新群号或邮箱。许可证、版权署名、作者姓名、原始来源证明、第三方依赖和运行兼容引用继续保留。测试中的虚构邮箱及 Git SSH 地址不属于对外联系方式。历史 Git 提交保持原样。
