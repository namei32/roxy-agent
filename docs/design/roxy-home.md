...
用户确认：先显示本地首页与可点击导航；当前页面优先加载；其余历史在后台同步；插件和首页区块独立更新。启动不再以所有插件或所有历史完成作为界面显示条件。

能力 owner 为客户端加载协调器、共享 WebUI 的插件挂载器及小屋插件自己的只读区块。consumer_scope 为 Android 与共享插件界面；runtime_patch 为 none，权威消息、插件代际和业务数据仍由 Core 所有。客户端可以完整实现此优化，不增加服务端 RPC。

`change_type: performance`，`semantic_delta: compatible`。Android 从校验过的本地资源恢复目录，认证后立即刷新；目录按插件公布资源 ready/error，当前 dashboard 可用桥接 assets 控制调整本地下载优先级。它不调用插件工具或修改数据。资源并发限制为 2，超时只影响对应插件。旧壳未提供 ready 时仍按完整目录兼容。

WebUI 不等待所有模块导入；每个模块独立提交，错误只显示在该插件内。本地缓存可先渲染，但断线时仍禁止远程查询和操作。home 标记的本地存储只是按服务端隔离的 UI 偏好，当前模块加载后重新核对，不作为能力授权。无 snapshot 时也保留本地导航，不因目录尚未完成自动跳转聊天。

历史同步优先当前会话，其他会话逐个补齐；尚未完成 canonical history 的会话，其 durable outbox 保留待发，历史完成后继续发送。保留原有游标、代际、去重与重建校验，不迁移、合并或删除用户会话。小屋日常、信箱、情绪各自提交返回值与错误，不等待整组结果。

验收：类型检查、移动端状态测试、浏览器慢模块/失败模块并行渲染测试、Android 缓存恢复/优先级/超时/会话恢复测试及同包手机启动验证。回滚为恢复之前的 WebUI release、Android 同签名构建和正式插件源版本；无数据 schema 变更。
