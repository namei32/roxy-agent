...
| Mac `~/Library/Application Support/Akashic/NotesBridge/receipts.sqlite3` | proposal accepted 增加本地 operation；create committed 增加 document mapping | `MacNotesReceiptStore` 推进 operation 到 executing/terminal，并以 request hash、document key、Note/folder 回执恢复；append 只验证既有 mapping，不保存标题或正文 | 当前不得自动删除；撤销 token 或卸载 launchd 不删回执 |
