...
| `data/notes-bridge.sqlite3` | 每个工具尝试增加 metadata-only operation；离线 skip 只保存哈希审计，不保存正文或延迟 payload | operation 按 proposed → proposal_accepted → commit_sent → terminal 更新，请求摘要不可改写 | 当前不得自动删除；unknown 和事故证据保留 | NotesBridgeBroker；operation/request/content hash、connection epoch 与终态 |
