...
| `plugin-data/apple_notes-<marketplace>/notes-bridge.sqlite3` | 每个工具尝试增加 metadata-only operation；离线 skip 只保存哈希审计，不保存正文或延迟 payload | operation 按 proposed → proposal_accepted → commit_sent → terminal 更新，请求摘要不可改写 | 当前不得自动删除；unknown 和事故证据保留 | 外部 Apple Notes Plugin/Broker；operation/request/content hash、connection epoch 与终态 |
| Mac `~/Library/Application Support/Akashic/NotesBridge/receipts.sqlite3` | proposal accepted 增加本地 operation；create committed 增加 document mapping | operation 推进到 executing/terminal；append 验证既有 mapping | 当前不得自动删除；撤销 token 或卸载 launchd 不删回执 | MacNotesReceiptStore；request hash、document key、Note/folder 回执，不保存标题或正文 |
