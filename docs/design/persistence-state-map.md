...
| `data/notes-bridge.sqlite3` | 每个工具尝试增加 metadata-only operation；离线 skip 只保存哈希审计，不保存正文或延迟 payload | `NotesBridgeBroker` 按 proposed → proposal_accepted → commit_sent → terminal 更新；operation、request/content hash、connection epoch 与终态是恢复证据，请求摘要不可改写 | 当前不得自动删除；unknown 和事故证据保留 |
| Mac `~/Library/Application Support/Roxy/NotesBridge/receipts.sqlite3`（旧目录存在时继续原位读取 `Akashic/NotesBridge`） | proposal accepted 增加本地 operation；create committed 增加 document mapping | `MacNotesReceiptStore` 推进 operation 到 executing/terminal，并以 request hash、document key、Note/folder 回执恢复；append 只验证既有 mapping，不保存标题或正文 | 当前不得自动删除；撤销 token 或卸载 launchd 不删回执，也不得自动移动旧目录 |
...
| `data/mobile/keys/current.json` | 新 keyset 原子发布当前版本、manifest 路径和 `runtime_identity = "roxy"`；历史文件没有该字段时继续作为旧身份读取 | master key 轮换只推进版本和 manifest；历史文件轮换时继续不写 Roxy marker，避免静默切换已配对设备的密钥命名空间与证书身份 | 当前没有自动删除协议；旧 keyset version 保留供回滚。owner 是 `KeysetManager`，恢复证据是 current 指针、不可变版本目录、server/TLS fingerprint 与既有配对记录 |
...
| `~/.roxy/auth.json` 与旧 `~/.akashic/auth.json` | 新的全局 JSON 凭据写入 Roxy 路径；默认 store 在新文件不存在时可读取旧文件，并在下一次明确写入时把读取到的完整文档发布到新路径 | Roxy 文件按 credential ID 原子更新；兼容读取不改写旧文件。workspace 模型凭据仍由 `model-registry.sqlite3` 拥有，不得回退这里 | 当前 store 没有通用删除 API；不得因已生成 Roxy 副本就删除旧文件。owner 是显式 JSON `CredentialStore`，恢复证据是旧文件、Roxy 原子写和 `before-write` 备份 |
| `~/.roxy-plugin/manifest.toml` 与旧 `~/.akashic-plugin/manifest.toml` | 新安装写 Roxy 根；Roxy 根不存在而旧根存在时，运行时继续使用旧 manifest | enable/disable 更新当前选中根内的 entry；`ROXY_PLUGIN_HOME` 优先于旧环境别名 | 明确卸载时移除对应 entry；这只减少安装清单和能力，不删除 workspace 内 plugin-data，也不自动移动或删除旧插件根 |
| `~/.roxy-plugin/cache/` 与旧 `~/.akashic-plugin/cache/` | 插件安装在当前选中根的 staging 校验后发布插件代码、Skill 和 MCP runtime | 更新版本时原子替换并可回滚当前安装事务；新旧根不自动合并 | 明确卸载可以删除当前根内代码与能力 cache；它不是外部 canonical source，也不授权删除 plugin-data 或另一兼容根 |
