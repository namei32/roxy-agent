...
移动端主动判断须在同一用户允许的手机会话范围内读取近期互动、运行/排队状态与已送达消息，不再固定只读取配置目标会话。私密、归档、验证或主动上下文被禁用的会话不进入内容候选；其聚合互动时间及忙碌状态仅用于降低打扰，不向模型暴露内容。其他渠道仍保持原用户/目标边界。

会话摘要应为带来源身份的有界只读投影，随原消息更新，不把未回复或失败状态自动当成待办。明确关联优先，相关性仅辅助筛选；模型关联发送目标前必须读取该会话。推送真正提交前再次检查新输入、忙碌、来源变更及目标有效性。主动反馈优先使用 canonical 消息身份，并沿明确的讨论来源解析跨会话归因；相似度不能用于跨会话猜测归属，重放不得重复产生反馈效果。
